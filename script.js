...
function printDeveloper() {
  ...
  let developer= data.filter((prof) => prof.profession==='developer')
  ...
  function removeAdmin() {
    ...
    const filtData = data.filter((person) => person.profession !== 'admin');
  ...
  function checkAgeAbove25() {
    const age25 = data.some((person) => person.age > 25);
  console.log(age25);
  ...
  function uniqueProfessions() {
    const prof = [...new Set(data.map((person) => person.profession))];
    console.log(prof);
